...
#         output = feat @ self.params[self.w_name] + self.params[self.b_name]
        if feat.shape[0] is None:
            feat = feat.reshape(1, self.input_dim)
        else:
            feat = feat.reshape((feat.shape[0]), self.input_dim)
        
        output = feat @ self.params[self.w_name] + self.params[self.b_name].T
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
...
#         self.grads[self.b_name] = dprev.sum(axis=0)
        dfeat = (dprev @ (self.params[self.w_name].T)).reshape(feat.shape)
        self.grads[self.w_name] = feat.reshape((feat.shape[0], self.input_dim)).T @ dprev
        self.grads[self.b_name] = dprev.sum(axis=0)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
...
class gelu(object):
    ...
    def forward(self, feat):
        ...
        # Store the results in the variable output provided above.                  #
        #############################################################################
        output = feat * 0.5 * (1 + np.tanh(np.sqrt(2 / np.pi) * (feat + 0.044715 * feat ** 3)))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        dfeat = dprev * (0.5 * np.tanh(0.0356774 * feat ** 3 + 0.797885 * feat) + 
                         0.5 + (0.0535161 * feat ** 3 + 0.398942 * feat) *
                         (np.cosh(0.0356774 * feat ** 3 + 0.797885 * feat)) ** -2)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class dropout(object):
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        if self.keep_prob != 0 and self.is_training:
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        log_probs = -np.log(logit[range(feat.shape[0]), label])
        loss = np.sum(log_probs)/feat.shape[0]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = logit
...
def softmax(feat):
    ...
    #############################################################################
    expv = np.exp(feat - np.max(feat, axis = 1, keepdims=True))
    scores = expv/expv.sum(axis=1, keepdims=True)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return scores
